...
function badgeModuleLabel(item: GamificationBadge): string | null {
  const text = `${item.badge_code || ""} ${item.badge_title || ""}`.toLowerCase();
...
function BadgeTile({ item }: { item: GamificationBadge }) {
  const progressTarget =
    typeof item.progress_target === "number" ? Math.max(item.progress_target, 0) : null;
  ...
  const progressLabel =
    progressCurrent !== null && progressTarget !== null
      ? `${progressTarget > 0 ? Math.min(progressCurrent, progressTarget) : progressCurrent}/${progressTarget}`
      : null;
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="text-sm font-semibold text-white">{item.badge_title}</div>
            {badgeModuleLabel(item) ? (
          ...
          <div className="mt-1 text-xs leading-5 text-slate-400">{item.badge_description}</div>
        </div>
        {item.unlocked ? (
  ...
  return (
    <Card className="cyber-card cyber-glow-border relative overflow-hidden rounded-3xl border border-white/10">
      ...
      <CardHeader className="relative px-4 pt-4 pb-2 sm:px-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="text-base text-white">{isArabic ? "Ù…Ø¬Ù…ÙˆØ¹Ø© Ø§Ù„Ø£ÙˆØ³Ù…Ø©" : "Badge Collection"}</CardTitle>
            <p className="mt-1.5 text-xs leading-5 text-slate-400">
              {isArabic ? "ØªØ¨Ù‚Ù‰ Ø§Ù„Ø¥Ù†Ø¬Ø§Ø²Ø§Øª Ø§Ù„Ù…ÙØªÙˆØ­Ø© Ø¸Ø§Ù‡Ø±Ø©ØŒ Ø¨ÙŠÙ†Ù…Ø§ ØªØ¹Ø±Ø¶ Ø§Ù„Ø£ÙˆØ³Ù…Ø© Ø§Ù„Ù…Ù‚ÙÙ„Ø© ØªÙ‚Ø¯Ù…Ù‡Ø§ Ø§Ù„Ù…Ø¨Ø§Ø´Ø±." : "Unlocked achievements stay visible, while locked badges show live progress."}
            </p>
          </div>
      ...
      <CardContent className="relative px-4 pb-4 sm:px-5">
        {items.length === 0 ? (
          <div className="rounded-2xl border border-white/8 bg-black/15 p-3 text-sm text-slate-400">
            {isArabic ? "Ø£ÙƒÙ…Ù„ ÙØ­ØµÙ‹Ø§ Ù…Ø¤Ù‡Ù„Ù‹Ø§ Ù„Ø¨Ø¯Ø¡ Ø¬Ù…Ø¹ Ø§Ù„Ø£ÙˆØ³Ù…Ø©." : "Complete an eligible scan to start earning badges."}
          </div>
        ) : (
